//! Developer settings page for SystemSettings (example content).
//!
//! Header with a gray hammer icon plus example rows (Developer Mode
//! toggle, API Logs). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const DEVELOPER_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `hammer.fill` icon, same artwork as the sidebar row icon.
fn developer_icon_path() -> Option<String> {
  sidebar_style_icon_path("hammer.fill", "developer", DEVELOPER_GRAY)
}
...
/// The Developer detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray hammer icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = developer_icon_path() {
  ...
  let title = markup_label(&lang::t("developer.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("developer.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
