//! Keyboard settings page for SystemSettings (example content).
//!
//! Header with a gray keyboard icon plus example rows (Key Repeat
//! toggle, Shortcuts). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const KEYBOARD_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `keyboard.fill` icon, same artwork as the sidebar row icon.
fn keyboard_icon_path() -> Option<String> {
  sidebar_style_icon_path("keyboard.fill", "keyboard", KEYBOARD_GRAY)
}
...
/// The Keyboard detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray keyboard icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = keyboard_icon_path() {
  ...
  let title = markup_label(&lang::t("keyboard.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("keyboard.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
