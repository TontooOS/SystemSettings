//! Desktop & Dock settings page for SystemSettings (example content).
//!
//! Header plus example rows (Wallpaper, Show Dock and Magnification
...
/// Black `menubar.dock.rectangle` icon, same artwork as the sidebar icon.
fn desktop_dock_icon_path() -> Option<String> {
  sidebar_style_icon_path("menubar.dock.rectangle", "desktop_dock", (0, 0, 0))
}
...
/// The Desktop & Dock detail page (example content, directly on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: blue icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = desktop_dock_icon_path() {
  ...
  let title = markup_label(&lang::t("desktop_dock.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("desktop_dock.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
  ...
  rows.append(&info_row(
    "desktop_dock.wallpaper",
    "desktop_dock.wallpaper.detail",
    pal.fg,
    pal.secondary,
    false,
  ));
  rows.append(&toggle_row(
    "desktop_dock.show_dock",
    true,
    "Show Dock",
    pal.fg,
    false,
