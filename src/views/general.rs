//! General settings page for SystemSettings (example content).
//!
//! Header with a gray gear plus example rows (About, Software Update).
...
/// Gray `gear` icon, same artwork as the sidebar row icon.
fn general_icon_path() -> Option<String> {
  sidebar_style_icon_path("gear", "general", GEAR_GRAY)
}
...
/// The General detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray gear icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = general_icon_path() {
  ...
  let title = markup_label(&lang::t("general.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("general.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
