//! Accessibility settings page for SystemSettings (example content).
//!
//! Header with a blue accessibility icon plus example rows (Display,
//! Reduce Motion toggle). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
/// Blue `figure.wave.circle` icon, same artwork as the sidebar row icon.
fn accessibility_icon_path() -> Option<String> {
  sidebar_style_icon_path("figure.wave.circle", "accessibility", WIFI_BLUE)
}
...
/// The Accessibility detail page (example content, directly on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: blue accessibility icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = accessibility_icon_path() {
  ...
  let title = markup_label(&lang::t("accessibility.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("accessibility.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
