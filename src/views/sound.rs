//! Sound settings page for SystemSettings (example content).
//!
//! Header with a pink speaker icon plus example rows (Output, Mute
...
const SOUND_PINK: (u8, u8, u8) = (255, 45, 85);

/// Pink `speaker.wave.3.fill` icon, same artwork as the sidebar row icon.
fn sound_icon_path() -> Option<String> {
  sidebar_style_icon_path("speaker.wave.3.fill", "sound", SOUND_PINK)
}
...
/// The Sound detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: pink speaker icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = sound_icon_path() {
  ...
  let title = markup_label(&lang::t("sound.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("sound.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
