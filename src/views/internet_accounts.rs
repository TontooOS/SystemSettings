//! Internet Accounts settings page for SystemSettings (example content).
//!
//! Header with a blue mail icon plus example rows (Tontoo Account,
//! Mail toggle). All text uses SF Pro Display and both `en_us` and
//! `de_de` strings.
...
/// Blue `mail.stack.fill` icon, same artwork as the sidebar row icon.
fn internet_accounts_icon_path() -> Option<String> {
  sidebar_style_icon_path("mail.stack.fill", "internet_accounts", WIFI_BLUE)
}
...
/// The Internet Accounts detail page (example content, on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: blue mail icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = internet_accounts_icon_path() {
  ...
  let title = markup_label(&lang::t("internet_accounts.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("internet_accounts.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
  ...
  rows.append(&info_row(
    "internet_accounts.account",
    "internet_accounts.account.detail",
    pal.fg,
    pal.secondary,
    false,
  ));
  rows.append(&toggle_row(
    "internet_accounts.mail",
    true,
    "Mail",
    pal.fg,
    true,
