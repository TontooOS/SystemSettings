//! Privacy & Security settings page for SystemSettings (example content).
//!
//! Header with a blue raised-hand icon plus example rows (Location
//! Services toggle, App Permissions). All text uses SF Pro Display and
...
/// Blue `hand.raised.fill` icon, same artwork as the sidebar row icon.
fn privacy_icon_path() -> Option<String> {
  sidebar_style_icon_path("hand.raised.fill", "privacy", WIFI_BLUE)
}
...
/// The Privacy & Security detail page (example content, on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: blue hand icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = privacy_icon_path() {
  ...
  let title = markup_label(&lang::t("privacy.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("privacy.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
