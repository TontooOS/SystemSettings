//! Users & Groups settings page for SystemSettings (example content).
//!
//! Header with a gray people icon plus example rows (Current User,
//! Guest User toggle). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const USERS_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `person.2.fill` icon, same artwork as the sidebar row icon.
fn users_icon_path() -> Option<String> {
  sidebar_style_icon_path("person.2.fill", "users", USERS_GRAY)
}
...
/// The Users & Groups detail page (example content, directly on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray people icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = users_icon_path() {
  ...
  let title = markup_label(&lang::t("users.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("users.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
