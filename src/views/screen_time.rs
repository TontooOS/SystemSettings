//! Screen Time settings page for SystemSettings (example content).
//!
//! Header with an indigo hourglass icon plus example rows (Downtime
//! toggle, App Limits). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const SCREENTIME_INDIGO: (u8, u8, u8) = (88, 86, 214);

/// Indigo `hourglass` icon, same artwork as the sidebar row icon.
fn screen_time_icon_path() -> Option<String> {
  sidebar_style_icon_path("hourglass", "screen_time", SCREENTIME_INDIGO)
}
...
/// The Screen Time detail page (example content, directly on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: indigo hourglass icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = screen_time_icon_path() {
  ...
  let title = markup_label(&lang::t("screen_time.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("screen_time.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
  ...
  rows.append(&info_row(
    "screen_time.app_limits",
    "screen_time.app_limits.detail",
    pal.fg,
    pal.secondary,
    true,
  ));
