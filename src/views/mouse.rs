//! Mouse & Trackpad settings page for SystemSettings (example content).
//!
//! Header with a gray cursor icon plus example rows (Tracking Speed,
//! Natural Scroll toggle). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const MOUSE_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `cursorarrow` icon, same artwork as the sidebar row icon.
fn mouse_icon_path() -> Option<String> {
  sidebar_style_icon_path("cursorarrow", "mouse", MOUSE_GRAY)
}
...
/// The Mouse & Trackpad detail page (example content, on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray cursor icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = mouse_icon_path() {
  ...
  let title = markup_label(&lang::t("mouse.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("mouse.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
