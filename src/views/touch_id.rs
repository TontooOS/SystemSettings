//! Touch ID & Password settings page for SystemSettings (example content).
//!
//! Header with a pink fingerprint icon plus example rows (Unlock with
//! Touch ID toggle, Passwords). All text uses SF Pro Display and both
...
const TOUCH_PINK: (u8, u8, u8) = (255, 45, 85);

/// Pink `touchid` icon, same artwork as the sidebar row icon.
fn touch_id_icon_path() -> Option<String> {
  sidebar_style_icon_path("touchid", "touch_id", TOUCH_PINK)
}
...
/// The Touch ID & Password detail page (example content, on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: pink fingerprint icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = touch_id_icon_path() {
  ...
  let title = markup_label(&lang::t("touch_id.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("touch_id.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
