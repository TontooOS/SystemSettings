//! Octo Cloud settings page for SystemSettings (example content).
//!
//! Header with an orange cloud icon plus example rows (Storage, Sync
//! toggle). All text uses SF Pro Display and both `en_us` and `de_de`
//! strings.
...
const OCTO_ORANGE: (u8, u8, u8) = (255, 107, 43);

/// Orange `icloud.fill` icon, same artwork as the sidebar row icon.
fn octo_cloud_icon_path() -> Option<String> {
  sidebar_style_icon_path("icloud.fill", "octo_cloud", OCTO_ORANGE)
}
...
/// The Octo Cloud detail page (example content, directly on screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: orange cloud icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = octo_cloud_icon_path() {
  ...
  let title = markup_label(&lang::t("octo_cloud.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("octo_cloud.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
