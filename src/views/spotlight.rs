...
const SPOTLIGHT_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `magnifyingglass` icon, same artwork as the sidebar row icon.
fn spotlight_icon_path() -> Option<String> {
  sidebar_style_icon_path("magnifyingglass", "spotlight", SPOTLIGHT_GRAY)
}
...
/// The Spotlight detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray search icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = spotlight_icon_path() {
  ...
  let title = markup_label(&lang::t("spotlight.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("spotlight.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
