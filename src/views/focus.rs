//! Focus settings page for SystemSettings (example content).
//!
//! Header with an indigo moon icon plus example toggle rows (Do Not
//! Disturb, Sleep). All text uses SF Pro Display and both `en_us` and
//! `de_de` strings.
...
const FOCUS_INDIGO: (u8, u8, u8) = (88, 86, 214);

/// Indigo `moon.fill` icon, same artwork as the sidebar row icon.
fn focus_icon_path() -> Option<String> {
  sidebar_style_icon_path("moon.fill", "focus", FOCUS_INDIGO)
}
...
/// The Focus detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: indigo moon icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = focus_icon_path() {
  ...
  let title = markup_label(&lang::t("focus.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("focus.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
