//! Wallpaper settings page for SystemSettings (example content).
//!
//! Header with a turquoise atom icon plus example rows (Current,
//! Auto Change toggle). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const WALLPAPER_TEAL: (u8, u8, u8) = (48, 176, 199);

/// Turquoise `atom` icon, same artwork as the sidebar row icon.
fn wallpaper_icon_path() -> Option<String> {
  sidebar_style_icon_path("atom", "wallpaper", WALLPAPER_TEAL)
}
...
/// The Wallpaper detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: turquoise atom icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = wallpaper_icon_path() {
  ...
  let title = markup_label(&lang::t("wallpaper.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("wallpaper.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
  ...
  rows.append(&info_row(
    "wallpaper.current",
    "wallpaper.current.detail",
    pal.fg,
    pal.secondary,
    false,
  ));
  rows.append(&toggle_row(
    "wallpaper.auto_change",
    false,
    "Auto Change",
    pal.fg,
    true,
