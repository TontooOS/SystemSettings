//! Printers settings page for SystemSettings (example content).
//!
//! Header with a gray printer icon plus example rows (Default Printer,
//! Double-Sided toggle). All text uses SF Pro Display and both `en_us`
//! and `de_de` strings.
...
const PRINTER_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `printer.fill` icon, same artwork as the sidebar row icon.
fn printers_icon_path() -> Option<String> {
  sidebar_style_icon_path("printer.fill", "printers", PRINTER_GRAY)
}
...
/// The Printers detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray printer icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = printers_icon_path() {
  ...
  let title = markup_label(&lang::t("printers.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("printers.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
  ...
  rows.append(&info_row(
    "printers.default",
    "printers.default.detail",
    pal.fg,
    pal.secondary,
    false,
  ));
  rows.append(&toggle_row(
    "printers.double_sided",
    true,
    "Double-Sided",
    pal.fg,
    true,
