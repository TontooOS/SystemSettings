//! Menu Bar settings page for SystemSettings (example content).
//!
//! Header with a gray menu bar icon plus example toggle rows (Clock,
//! Spotlight). All text uses SF Pro Display and both `en_us` and `de_de`
//! strings.
...
const MENU_GRAY: (u8, u8, u8) = (142, 142, 147);

/// Gray `switch.2` icon, same artwork as the sidebar row icon.
fn menu_bar_icon_path() -> Option<String> {
  sidebar_style_icon_path("switch.2", "menu_bar", MENU_GRAY)
}
...
/// The Menu Bar detail page (example content, directly on the screen).
pub(crate) fn build_page() -> gtk::Widget {
  ...
  // Header row: gray menu bar icon, title + subtitle.
  let header = gtk::Box::new(gtk::Orientation::Horizontal, 10);
  header.set_hexpand(true);

  if let Some(icon_path) = menu_bar_icon_path() {
  ...
  let title = markup_label(&lang::t("menu_bar.title"), 17, "bold", pal.fg);
  title.set_halign(gtk::Align::Start);
  title.set_xalign(0.0);
  titles.append(&title);
  let subtitle = markup_label(
    &lang::t("menu_bar.header.subtitle"),
    13,
    "normal",
    pal.secondary,
  );
